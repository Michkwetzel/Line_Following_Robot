...
if write_video
    writerObj = VideoWriter('wheeled_robot');
    open(writerObj);
end
...
%%{
dt = out.tout(end)/size(out.q, 1);
for i = 1:every_nth:length(x)
    title({strcat('Awesome-O Robot - t=', num2str((i-1)*dt), 's')},...
        'FontWeight','bold','FontSize',24,'FontName','AvantGarde');
